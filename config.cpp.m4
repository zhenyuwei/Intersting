...
dnl add dynamic lib to PHP
   ...
   PHP_ADD_LIBRARY(stdc++, 1, PHP_TESTL_SHARED_LIBADD) dnl c++ dynamic lib
   PHP_REQUIRE_CXX() dnl c++ dynamuc lib
   PHP_SUBST(PHP_TESTL_SHARED_LIBADD)
  
dnl add c file in Makefile
   PHP_NEW_EXTENSION(php_test, php_test.cpp php_test1.cpp php_test2.cpp, $ext_shared)
fi
